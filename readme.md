...
Or, after tables exist, apply the committed combined seed file:

```powershell
psql $env:DATABASE_URL -f db_scripts/tables/001_card_table.sql
psql $env:DATABASE_URL -f migration/001_migration.sql
```
...
It creates `db_scripts/seeds/to_migration.sql`; use that file only after running the script.
